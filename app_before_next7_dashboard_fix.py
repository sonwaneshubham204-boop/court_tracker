...
def home():
    ...
    court_counts = {
        court_no: Case.query.filter_by(court_no=court_no).count()
        for court_no in (1, 2, 3)
    }
    court_1_count = court_counts[1]
    court_2_count = court_counts[2]
    court_3_count = court_counts[3]
    ...
    stage_counts = {
        stage: Case.query.filter_by(case_stage=stage).count()
        for stage in stage_names
    }
    notice_count = stage_counts["Notice"]
    admission_count = stage_counts["Admission"]
    hearing_count = stage_counts["Hearing"]
    evidence_count = stage_counts["Evidence"]
    cross_examination_count = stage_counts["Cross Examination"]
    arguments_count = stage_counts["Arguments"]
    final_arguments_count = stage_counts["Final Arguments"]
    final_hearing_count = stage_counts["Final Hearing"]
    judgment_count = stage_counts["Judgment"]
    order_count = stage_counts["Order"]
...
def _case_form_data():
    decision_date = _parse_date(request.form.get("decision_date", "").strip())
    stage = request.form.get("case_stage", "").strip()
    other = request.form.get("case_stage_other", "").strip()
    return {
        "case_no": request.form.get("case_no", "").strip(),
        "crn_no": request.form.get("crn_no", "").strip() or None,
        "advocate_name": request.form.get("advocate_name", "").strip() or None,
        "case_disposed": "Yes" if decision_date else (request.form.get("case_disposed", "").strip() or "No"),
        "decision_date": decision_date,
        "court_no": int(request.form.get("court_no")),
        "parties": request.form.get("parties", "").strip(),
        "case_stage": stage,
        "case_stage_other": other if stage == "Other" and other else None,
        "next_hearing_date": _parse_date(request.form.get("next_hearing_date", "").strip()),
    }

def _save_evidence_files(evidence, uploaded_files):
...
def _remove_evidence_files(evidence):
    for evidence_file in evidence.files:
        _remove_file(evidence_file.file_name)

def _apply_bulk_update(cases, hearing_date=None, stage=None):
    for case in cases:
...
def add_case():
    if request.method == "POST":
        db.session.add(Case(**_case_form_data()))
        db.session.commit()
...
def edit_case(id):
    case = Case.query.get_or_404(id)
    if request.method == "POST":
        for field, value in _case_form_data().items():
...
def add_evidence(id):
    ...
    _save_evidence_files(new_evidence, uploaded_files)
...
def delete_evidence(id):
    ...
    _remove_evidence_files(evidence)
...
@login_required
def _parse_excel_hearing_date(value):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            for row in worksheet.iter_rows(
                ...
                hearing_date = _parse_excel_hearing_date(hearing_date)
...
def bulk_update():
    ...
    _apply_bulk_update(_bulk_cases(), hearing_date, stage)
...
def bulk_date():
    ...
    _apply_bulk_update(_bulk_cases(), hearing_date=d)
    db.session.commit()
    flash("Next hearing date updated for selected cases.", "success")
    return redirect(request.referrer or url_for("case_list"))
...
def bulk_stage():
    ...
    _apply_bulk_update(_bulk_cases(), stage=stage)
    db.session.commit()
    flash("Case stage updated for selected cases.", "success")
    return redirect(request.referrer or url_for("case_list"))
